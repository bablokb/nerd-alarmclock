...
class Alarm(object):
  ...
  def read(self,nr):
    ...
    self.tlo         = self._settings.get("alarm.%s.tlo"       % nr)
    ...
    self.disp        = self.disp_cfg['name']
    self.disp_dur    = self.disp_cfg['duration']
    self.disp_delay  = (str(self.disp_cfg['delay'])
                        if self.disp_cfg.has_key('delay') else "0")
    self.disp_text   = (str(self.disp_cfg['text'])
                        if self.disp_cfg.has_key('text') else "")
...
class BotThread(Thread):
    ...
    if value == "global":
      ...
      response = _(
        ...
        " fadein:   %s\n"
             ...
             _(alarm.state),
             alarm.time, alarm.tlo,
             alarm.snooze,
             alarm.days,
             alarm.led, alarm.led_dur, alarm.led_delay,
             alarm.disp, alarm.disp_text, alarm.disp_dur, alarm.disp_delay,
  ...
  def _do_alarm_attr_callback(self,query_id,chat_id,value):
    ...
    buttons.append(button)
    button = [InlineKeyboardButton(text=_("toggle lights off: %s") % alarm.tlo,
                      callback_data='{"name":"alarm_tlo", "value": ""}')]
    buttons.append(button)
    ...
    button = [InlineKeyboardButton(text=_("Display: %s") % alarm.disp,
                      callback_data='{"name":"alarm_disp", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text=_("duration: %s") % alarm.disp_dur,
                       callback_data='{"name":"alarm_disp_dur", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text=_("delay: %s") % alarm.disp_delay,
                       callback_data='{"name":"alarm_disp_delay", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text=_("text: %s") % alarm.disp_text,
                       callback_data='{"name":"alarm_disp_text", "value": ""}')]
    buttons.append(button)
  ...
  def _do_alarm_state_callback(self,query_id,chat_id,value):
    ...
    if value:
      ...
      self._bot.sendMessage(chat_id,_("new state: %s") % self._alarm.state)
    else:
      # show keyboard
  ...
  # set alarm tlo callback   ---------------------------------------------

  def _do_alarm_tlo_callback(self,query_id,chat_id,value):
    """ set alarm toggle-lights-off flag """

    self._settings.log.msg("BotThread: setting alarm tlo.")
    self._bot.answerCallbackQuery(query_id)

    if value:
      # set value of alarm-state
      self._alarm.tlo = value
      self._settings.set("alarm.%s.tlo" % self._alarm.nr,value)
      self._bot.sendMessage(chat_id,_("new state: %s") % self._alarm.tlo)
    else:
      # show keyboard
      buttons = [[InlineKeyboardButton(text=_("enable"),
                   callback_data='{"name":"alarm_tlo", "value": "enabled"}')],
               [InlineKeyboardButton(text=_("disable"),
                   callback_data='{"name":"alarm_tlo", "value": "disabled"}')]
              ]
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Set toggle lights-off-mode of alarm"),reply_markup=keyboard)

  # set alarm snooze-delay callback   -------------------------------------
  ...
  # set alarm display callback   ----------------------------------------------

  def _do_alarm_disp_callback(self,query_id,chat_id,value):
    """ set alarm display """

    self._settings.log.msg("BotThread: setting alarm display")
    self._bot.answerCallbackQuery(query_id)

    if value:
      # set value of display-alarm
      if value == '_':
        self._alarm.disp_cfg['name'] = ''
        self._bot.sendMessage(chat_id,_("selected display-alarm: %s") %
                                                                  _("No alarm"))
      else:
        self._alarm.disp_cfg['name'] = value
        self._bot.sendMessage(chat_id,_("selected display-alarm: %s") % value)

      if value != 'text':
        # clear attribute text
        self._alarm.disp_cfg['text'] = ''
      self._settings.set("alarm.%s.display" % self._alarm.nr,self._alarm.disp_cfg)

    else:
      # show list of possible alarms
      buttons = []
      for alarm in self._settings.get("_display_alarms"):
        button = InlineKeyboardButton(text=alarm,
                   callback_data='{"name":"alarm_disp", "value": "%s"}' % alarm)
        buttons.append([button])

      # append choice "no alarm"
      button = InlineKeyboardButton(text=_("No alarm"),
               callback_data='{"name":"alarm_disp", "value": "_"}')
      buttons.append([button])

      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Select display-alarm"),reply_markup=keyboard)
  ...
  # set display alarm duration callback   --------------------------------------

  def _do_alarm_disp_dur_callback(self,query_id,chat_id,value):
    """ set display alarm duration """

    self._settings.log.msg("BotThread: setting display alarm duration")
    self._bot.answerCallbackQuery(query_id)

    if not value:
      # reset and show keyboard
      self._alarm.disp_dur_new = ""
      keys = [str(i+1) for i in range(9)]
      keys += [' ','0','ok']
      buttons = self._get_keypad(keys,'alarm_disp_dur')
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Set display alarm duration"),
                            reply_markup=keyboard)
      return

    if value == 'ok':
      # set value of alarm-duration
      self._alarm.disp_cfg['duration']   = self._alarm.disp_dur_new
      self._settings.set("alarm.%s.display" % self._alarm.nr,self._alarm.disp_cfg)
      self._bot.sendMessage(chat_id,_("new duration: %s") % self._alarm.disp_dur_new)
    elif value == " ":
      pass
    else:
      # save digit
      self._alarm.disp_dur_new += value

  # set display text callback   --------------------------------------------

  def _do_alarm_disp_text_callback(self,query_id,chat_id,value):
    """ set display alarm text """

    self._settings.log.msg("BotThread: callback display alarm text")
    self._bot.answerCallbackQuery(query_id)

    if self._alarm.disp_cfg['name'] == 'text':
      self._msg_handler = self._set_alarm_disp_text
      self._bot.sendMessage(chat_id,_("Set display alarm text"))
    else:
      self._bot.sendMessage(chat_id,_("No text selection for this alarm-type"))

  # save display text   -----------------------------------------------------

  def _set_alarm_disp_text(self,chat_id,text):
    """ save display alarm text """

    self._settings.log.msg("BotThread: set display alarm text")
    self._alarm.disp_cfg['text'] = text
    self._settings.set("alarm.%s.display" % self._alarm.nr,self._alarm.disp_cfg)
    self._bot.sendMessage(chat_id,_("new text: %s") % text)
    self._msg_handler = None

  # set display delay callback   --------------------------------------------

  def _do_alarm_disp_delay_callback(self,query_id,chat_id,value):
    """ set display alarm delay """

    self._settings.log.msg("BotThread: setting display alarm delay")
    self._bot.answerCallbackQuery(query_id)

    if not value:
      # reset and show keyboard
      self._alarm.disp_delay_new = ""
      keys = [str(i+1) for i in range(9)]
      keys += [' ','0','ok']
      buttons = self._get_keypad(keys,'alarm_disp_delay')
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Set display alarm delay"),
                            reply_markup=keyboard)
      return

    if value == 'ok':
      # set value of alarm-delay
      self._alarm.disp_cfg['delay']   = self._alarm.disp_delay_new
      self._settings.set("alarm.%s.display" % self._alarm.nr,self._alarm.disp_cfg)
      self._bot.sendMessage(chat_id,_("new delay: %s") % self._alarm.disp_delay_new)
    elif value == " ":
      pass
    else:
      # save digit
      self._alarm.disp_delay_new += value
  ...
  def _handle_message(self,msg):
    ...
    if self._msg_handler:
      self._msg_handler(chat_id,text)
    else:
