...
class Alarm(Object):
  ...
  def read(self,nr):
    ...
    self.dur       = self.led_cfg['duration']
    self.sound     = self.sound_cfg['name']
    self.volume    = self.sound_cfg['volume']
    # we use only one duration for led and sound
...
class BotThread(Thread):
  ...
  def _do_settings(self,chat_id,text):
    ...
    self._bot.sendMessage(chat_id,_("sorry, not implemented yet!"))
  ...
  def _do_alarm_attr_callback(self,query_id,chat_id,value):
    ...
    # create keyboard
    buttons = []
    button = [InlineKeyboardButton(text="state: %s" % alarm.state,
                     callback_data='{"name":"alarm_state", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="time: %s" % alarm.time,
                      callback_data='{"name":"alarm_time", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="days: %s" % alarm.days,
                      callback_data='{"name":"alarm_days", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="LED: %s" % alarm.led,
                      callback_data='{"name":"alarm_leds", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="Sound: %s" % alarm.sound,
                      callback_data='{"name":"alarm_sounds", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="duration: %s" % alarm.dur,
  ...
  def _do_alarm_state_callback(self,query_id,chat_id,value):
    ...
    if value:
      ...
      buttons = [[InlineKeyboardButton(text=_("enable"),
               ...
               [InlineKeyboardButton(text=_("disabled"),
  ...
  def _do_alarm_sounds_callback(self,query_id,chat_id,value):
    ...
    if value:
      ...
      self._alarm.sound_cfg['name'] = value
      self._settings.set("alarm.%s.sound" % self._alarm.nr,self._alarm.sound_cfg)
    else:
      # show list of possible alarms (two columns)
      buttons = []
      alist = self._settings.get("_sound_alarms"):
      for alarm1,alarm2 in zip(alist,alist[1:])[::2]:
        button1 = InlineKeyboardButton(text=alarm2,
               callback_data='{"name":"alarm_sounds", "value": "%s"}' % alarm1)
        button2 = InlineKeyboardButton(text=alarm2,
               callback_data='{"name":"alarm_sounds", "value": "%s"}' % alarm2)
        buttons.append([button1,button2])

      # we might need to add a last element (if alist has uneven elements)
      if len(alist) // 2:
        alarm = alist[-1]
        button = InlineKeyboardButton(text=alarm,
               callback_data='{"name":"alarm_sounds", "value": "%s"}' % alarm)
        buttons.append([button])
  ...
  def _do_alarm_dur_callback(self,query_id,chat_id,value):
    ...
    if not value:
      ...
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Set alarm duration"),reply_markup=keyboard)
      return

    if value == 'ok':
      # set value of alarm-duration
      self._alarm.led_cfg['duration']   = self._alarm.dur_new
      self._settings.set("alarm.%s.led" % self._alarm.nr,self._alarm.led_cfg)

      self._alarm.sound_cfg['duration'] = self._alarm.dur_new
      self._settings.set("alarm.%s.sound" % self._alarm.nr,self._alarm.sound_cfg)
      self._bot.sendMessage(chat_id,_("new duration: %s") % self._alarm.dur_new)
    elif value == " ":
      pass
    else:
      # save digit
      self._alarm.dur_new += value
