...
class Alarm(object):
  ...
  def read(self,nr):
    ...
    self.tdm         = self._settings.get("alarm.%s.tdm"       % nr)
...
class BotThread(Thread):
  ...
  def _do_alarm_attr_callback(self,query_id,chat_id,value):
    ...
    buttons.append(button)
    button = [InlineKeyboardButton(text=_("turn on day-mode: %s") % alarm.tdm,
                      callback_data='{"name":"alarm_tdm", "value": ""}')]
    buttons.append(button)
  ...
  # set alarm tdm callback   ---------------------------------------------

  def _do_alarm_tdm_callback(self,query_id,chat_id,value):
    """ set alarm toggle-day-mode flag """

    self._settings.log.msg("BotThread: setting alarm tdm.")
    self._bot.answerCallbackQuery(query_id)

    if value:
      # set value of alarm-state
      self._alarm.tdm = value
      self._settings.set("alarm.%s.tdm" % self._alarm.nr,value)
    else:
      # show keyboard
      buttons = [[InlineKeyboardButton(text=_("enable"),
                   callback_data='{"name":"alarm_tdm", "value": "enabled"}')],
               [InlineKeyboardButton(text=_("disable"),
                   callback_data='{"name":"alarm_tdm", "value": "disabled"}')]
              ]
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,_("Set toggle day-mode of alarm"),reply_markup=keyboard)
