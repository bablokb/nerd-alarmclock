...
import time, ConfigParser, traceback, re, os, signal, json, pprint
...
class BotThread(Thread):
  ...
  def _do_set_alarm(self,chat_id,text):
    ...
    for i in range(self._settings.MAX_ALARMS):
      ...
      button = InlineKeyboardButton(text=nr,
                       callback_data='{"name":"alarm_attr", "value": %s}' % nr)
  ...
  def _do_alarm_attr_callback(self,query_id,chat_id,value):
    ...
    # create keyboard
    buttons = []
    button = [InlineKeyboardButton(text="state: %s" % self._alarm_state,
                     callback_data='{"name":"alarm_state", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="time: %s" % self._alarm_time,
                      callback_data='{"name":"alarm_time", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="days: %s" % self._alarm_days,
                      callback_data='{"name":"alarm_days", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="LED: %s" % self._alarm_led,
                      callback_data='{"name":"alarm_leds", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="duration: %s" % self._alarm_dur,
                       callback_data='{"name":"alarm_dur", "value": ""}')]
    buttons.append(button)

    keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
    self._bot.sendMessage(chat_id,"Select attribute",reply_markup=keyboard)
  ...
  def _do_alarm_state_callback(self,query_id,chat_id,value):
    ...
    if value:
      ...
      # show keyboard
      buttons = [[InlineKeyboardButton(text="enable",
                   callback_data='{"name":"alarm_state", "value": "enabled"}')],
               [InlineKeyboardButton(text="disabled",
                   callback_data='{"name":"alarm_state", "value": "disabled"}')]
              ]
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,"Change state of alarm",reply_markup=keyboard)
  ...
  def _do_alarm_time_callback(self,query_id,chat_id,value):
    ...
    if not value:
      ...
      self._alarm_time_new = ""
      keys = [str(i+1) for i in range(9)]
      keys += [':','0','ok']
      buttons = self._get_keypad(keys,'alarm_time')
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,"Set alarm time",reply_markup=keyboard)
      return

    if value == 'ok':
      # set value of alarm-time
      self._alarm_time = self._alarm_time_new
      self._settings.set("alarm.%s.time" % self._alarm_nr,self._alarm_time)
      self._bot.sendMessage(chat_id,"new time: %s" % self._alarm_time)
    else:
      # save digit
      self._alarm_time_new = self._alarm_time_new + value
      if len(self._alarm_time_new) > 5:
        self._alarm_time_new = self._alarm_time_new[1:]
  ...
  def _do_alarm_days_callback(self,query_id,chat_id,value):
    ...
    if not value:
      # reset and show keyboard
      self._alarm_days_new = []
      buttons = self._get_keypad(DAYS,'alarm_days')
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,"Set alarm days",reply_markup=keyboard)
      return

    if value == 'ok':
      self._alarm_days = []
      for day in self._alarm_days_new:
        nr = 1 + DAYS.index(day)
        nr = 7 if nr == 8 else nr
        self._alarm_days.append(nr)
      self._settings.set("alarm.%s.days" % self._alarm_nr,self._alarm_days)
      self._bot.sendMessage(chat_id,"selected days: %s" % self._alarm_days)
    elif value == " ":
      pass
    else:
      # save in list
      self._alarm_days_new += [value]
  ...
  # set alarm duration callback   ---------------------------------------

  def _do_alarm_dur_callback(self,query_id,chat_id,value):
    """ set alarm duration """

    self._settings.log.msg("BotThread: setting alarm duration")
    self._bot.answerCallbackQuery(query_id)
    ...
    if not value:
      ...
      keys = [str(i+1) for i in range(9)]
      keys += [' ','0','ok']
      buttons = self._get_keypad(keys,'alarm_dur')
      keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
      self._bot.sendMessage(chat_id,"Set alarm duration",reply_markup=keyboard)
      return

    if value == 'ok':
      # set value of alarm-duration
      self._led_cfg['duration'] = self._alarm_dur_new
      self._settings.set("alarm.%s.led" % self._alarm_nr,self._led_cfg)
      self._bot.sendMessage(chat_id,"new duration: %s" % self._alarm_dur_new)
    elif value == " ":
      pass
    else:
      # save digit
      self._alarm_dur_new += value
