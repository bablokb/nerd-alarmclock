...
class BotThread(Thread):
  ...
  def _do_reboot(self,chat_id,text):
    ...
    self._bot.sendMessage(chat_id,"Please confirm reboot:",reply_markup=keyboard)

  # callback-handler for /reboot   -----------------------------------------

  def _do_reboot_callback(self,chat_id,value):
    """ callback-handler for the /reboot command """

    if value == 'no':
      self._bot.answerCallbackQuery(chat_id,"canceled reboot")
    else:
      self._bot.answerCallbackQuery(chat_id,"reboot the system")
