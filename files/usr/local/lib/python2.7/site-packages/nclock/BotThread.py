...
class Alarm(object):
  ...
  def read(self,nr):
    ...
    # we use only one duration for led, display and sound
...
class BotThread(Thread):
  ...
  def _do_alarm_attr_callback(self,query_id,chat_id,value):
    ...
    buttons.append(button)
    button = [InlineKeyboardButton(text="volume: %s" % alarm.vol,
                       callback_data='{"name":"alarm_vol", "value": ""}')]
    buttons.append(button)
    button = [InlineKeyboardButton(text="fadein: %s" % alarm.fadein,
  ...
  def _do_alarm_dur_callback(self,query_id,chat_id,value):
    ...
    if value == 'ok':
      ...
      self._alarm.disp_cfg['duration']   = self._alarm.dur_new
