...
class BotThread(Thread):
  ...
  def __init__(self,settings):
    ...
    self.__stop_event = settings.stop_event
    self.__bot_id     = settings.config_parser.get("BOT","bot_id")
    self.__user_id    = settings.config_parser.getint("BOT","user_id")
    ...
    # create bot
    self.__bot = telepot.Bot(self.__bot_id)
  ...
  def _handle_updates(self,update):
    ...
    # we only support private text messages and only messages from self.__user_id
    chat = update['message']['chat']
    if chat['type'] != "private" or chat['id'] != self.__user_id:
      return
    else:
      msg = update['message']
  ...
  def run(self):
    ...
    offset = None
    while not self.__stop_event.wait(BotThread.POLL_INTERVAL):
      print "polling ..."
      try:
