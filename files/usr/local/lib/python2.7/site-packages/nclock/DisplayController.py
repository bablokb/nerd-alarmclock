...
class DisplayController(object):
  ...
  def __init__(self,settings):
    ...
    settings.set("_display_modes",modes)
    settings.log.msg("DisplayController: available modes: %r" % (modes,))
  ...
  def on_lights_off(self,name,old,new):
    """ process lights_off-change events (update display)"""
    self._settings.log.msg("DisplayController: on_lights_off(%s,%s)" % (old,new))
    self.set_time()
  ...
  # --- alarm with running day, date and time   ----------------------------

  def alarm_daydatetime(self,stop_me,duration=1,text=None,delay=0):
    """ delegate to alarm_text with string from day, date and time """

    duration = int(duration)
    self._settings.log.msg("DisplayController: show day-date-time as alarm-text")

    # query datetime-text and replace some values
    txt = time.strftime("%a %x %H-%M   ")
    txt = text.replace('.','-')

    # call generic alarm_text
    self.alarm_text(stop_me,duration=duration,text=txt,delay=delay)
