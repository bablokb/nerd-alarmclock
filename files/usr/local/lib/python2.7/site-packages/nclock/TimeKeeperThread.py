...
class TimeKeeperThread(Thread):
  ...
  # --- notifiy time listeners   --------------------------------------------

  def _notify_time_listeners(self,old,new):
    """ notify parties interessted in time change events """

    self._settings.log.msg("notifying time-listeners: old: %s, new: %s" % (old,new))

    for func in self._settings.time_change_listener:
      func(old,new)

  # --- notifiy day listeners   ---------------------------------------------

  def _notify_date_listeners(self,old,new):
    """ notify parties interessted in date change events """

    self._settings.log.msg("notifying date-listeners: old: %s, new: %s" % (old,new))

    for func in self._settings.date_change_listener:
      func(old,new)
  ...
  def run(self):
    ...
    while True:
      ...
      if old_time[1] != new_time[1]:
        self._notify_time_listeners(old_time[1],new_time[1])
      if old_time[0] != new_time[0]:
        self._notify_date_listeners(old_time[0],new_time[0])
      old_time = new_time
