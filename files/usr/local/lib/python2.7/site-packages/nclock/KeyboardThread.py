...
class KeyboardThread(Thread):
  ...
  def run(self):
    ...
    pipe = open(KeyboardThread.GPIO_POLL_PIPE,"r")
    poll_obj = select.poll()
    poll_obj.register(pipe,select.POLLPRI|select.POLLIN)
    ...
    while True:
      ...
      for (fd,event) in poll_result:
        self._settings.log.msg("processing fd %s (event: %d)" % (fd,event))
  ...
  def change_alarm_state(self,button):
    ...
    self._settings.log.msg("changing state of button %1" % button)

    alarm = self._settings.get("alarm"+button)
    state = alarm.state
    ...
    self._settings.set("alarm"+button,alarm)
