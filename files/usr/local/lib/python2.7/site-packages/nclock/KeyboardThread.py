...
class KeyboardThread(Thread,KeyController):
  ...
  # buttons
  BUTTONS = ['BTN1','BTN2','BTN3','BTN4','SNZ','EK1_SW','EK2_SW']
  ...
  def _create_map(self):
    ...
    self._gpio_map = {}
    for btn in KeyboardThread.BUTTONS:
      gpio = self._settings.get_value('GPIO',btn,None)
    ...
    self.METHODS = [
      ...
      # long press
      { ('BTN1','DEF'):       self._toggle_state,
        ('BTN1','OVL_BTN1'):  self._toggle_state
      }]
  ...
  def _process_event(self,event):
    ...
    # ignore high-state
    if state == "1":
      self._settings.log.msg("KeyboardThread: GPIO %s, ignoring state 1" % gpio)
      return
  ...
  def _toggle_state(self,btn):
    ...
    self._settings.log.msg("KeyboardThread: _toggle_state() for button %s" % btn)
    # toggle overlay state
    if self._state == "DEF":
      self._state = "OVL_%s" % btn
      self._settings.set("_do_config","on")
      self._settings.log.msg("KeyboardThread: changed state to %s" % self._state)
      ...
      self._settings.log.msg("KeyboardThread: ignoring invalid state '%s'" %
                             self._state)
