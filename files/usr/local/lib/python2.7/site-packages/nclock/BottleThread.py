...
class BottleThread(Thread):
  ...
  def _add_rules(self):
    ...
    self._app.route("/time", callback=self.get, method='GET')
    self._app.route("/time", callback=self.post, method='POST')
  ...
  def get(self):
    ...
    return json.dumps({"time": str(self._settings.get(cfg))})

  def post(self):
    cfg  = "alarm.1.time"
    self._settings.set(cfg,request.forms.get('time'))
