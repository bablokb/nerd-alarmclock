...
class SoundController(object):
  ...
  def __init__(self,settings):
    ...
    self._amixer_opts = settings.get_value('SOUND','amixer_opts','')
  ...
  def _setup_channel_lists(self):
    ...
    # save in settings
    self._settings.set("_radio_channel_lists",self._channel_lists,notify=False)
    self._settings.set("_radio_channels",self._channels,notify=False)
  ...
  def _get_volume(self):
    ...
    try:
      cmd = "amixer %s get %s|grep -o [0-9]*%%|sed 's/%%//'| head -n 1" % (self._amixer_opts,self._mixer)
  ...
  def _set_volume(self,volume):
    ...
    try:
      args = shlex.split("amixer %s -q set %s %s%%" % (self._amixer_opts,self._mixer,volume))
  ...
  def play_sound(self,stop_me,name=None,fadein=0,duration=10,volume=40,delay=0):
    ...
    if delay > 0:
      self._settings.log.msg(
        "SoundController: delaying alarm for %d minutes" % delay)
  ...
  def on_list(self,name,old,new):
    ...
    self._settings.set("radio.current.list",name,notify=False)
