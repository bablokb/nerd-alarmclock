...
class FlaskThread(Thread):
  ...
  def __init__(self, settings, port):
    ...
    self._app.root_path = settings.get_value('WEB', 'document_root', '/var/www/html')
    self._app.add_url_rule("/", view_func=IndexAPI.as_view('index',self._settings, self._app))
...
class IndexAPI(MethodView):
  ...
  def get(self):
    ...
    self._settings.log.msg("WebThread: /")
...
class TimeAPI(MethodView):
  ...
  def get(self):
    ...
    self._settings.log.msg("WebThread: /time GET")
    cfg  = "alarm.1.time"
    return jsonify(time=self._settings.get(cfg))


  def post(self):
    """ handle post requests """

    self._settings.log.msg("WebThread: /time POST")
    cfg  = "alarm.1.time"
