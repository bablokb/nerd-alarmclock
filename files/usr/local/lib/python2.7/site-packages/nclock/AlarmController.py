...
class AlarmController(object):
  ...
  def __init__(self,settings):
    ...
    self._time       = time.strftime("%Y:%m:%d %H:%M").split()
    self._init_alarms(self._time[1])

    # we need to register for changes in alarm time
    settings.add_timechange_listener(self.on_time)

    # register for alarm-time changes
    self._settings.add_settings_listener('alarm.1.time',self.on_alarm_time)
    self._settings.add_settings_listener('alarm.2.time',self.on_alarm_time)
    self._settings.add_settings_listener('alarm.3.time',self.on_alarm_time)
    self._settings.add_settings_listener('alarm.4.time',self.on_alarm_time)

    # register for alarm-state changes
    self._settings.add_settings_listener('alarm.1.state',self.on_alarm_state)
    self._settings.add_settings_listener('alarm.2.state',self.on_alarm_state)
    self._settings.add_settings_listener('alarm.3.state',self.on_alarm_state)
    self._settings.add_settings_listener('alarm.4.state',self.on_alarm_state)

  # --- initialize alarms   ------------------------------------------------

  def _init_alarms(self,now):
    """ Initialize alarms """
  ...
  def on_time(self,old,new):
    ...
    if self._time[0] != new[0]:             # new day!
      self._init_alarms(new[1])

    self._settings.log.msg("checking alarms at %s" % new)
    for i in range(4):
      nr = str(i+1)
      if self._alarms[nr].time <= new[1] and self._alarms[nr].pending:
  ...
  def _fire_alarm(self,nr):
    ...
    alarm_threads = []
    for p in self._settings.alarm_provider:
      alarm_threads.append(p(nr,self._alarms[nr]))
