...
class KeyController(object):
  ...
  def toggle_day_mode(self,btn):
    """ toggle day mode """

    self._settings.log.msg("KeyController: toggle_day_mode()")
    old_mode = self._settings.get("_day_mode")
    if old_mode == "day":
      new_mode = "night"
    else:
      new_mode = "day"

    # check if we need to set override-mode
    day_start = self._settings.get("day.start")
    day_end   = self._settings.get("day.end")
    now       = time.strftime("%H:%M")
    if day.start <= now and now <= day.end:
      if new_mode == "day":
        override = False
      else:
        override = True
    ...
    self._settings.set("_day_mode_override",override)
