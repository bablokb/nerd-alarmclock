...
class LedController(object):
  ...
  def _set_brightness(self,value=None):
    ...
    self._settings.log.msg("LED: Setting brightness to: %d" % self._brightness)
    if not simulate:
      with self._lock:
  ...
  def alarm_police(self,stop_me,duration=5,rgb=[0,0,255]):
    """ simulate police signal (modified from blinkt-example larson.py) """
    
    duration = int(duration)

    self._settings.log.msg("running LED-alarm 'police' for %s minutes" % duration)
