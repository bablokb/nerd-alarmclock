...
class EncoderKnobThread(Thread,KeyController):
  ...
  def __init__(self,id,settings):
    ...
    self._map = { (1,0): self.radio_prev_channel,
                  (1,1): self.radio_next_channel,
                  (2,0): self.radio_volume_down,
  ...
  def _configure(self,msg_header):
    ...
    # setup data pin
    self._settings.log.msg(msg_header + "setting up data-pin (%s)" % self._data)
    ...
    self._write_value(gpio_dir  + 'edge', 'both')
    self._fd_data      = open(gpio_dir + 'value', 'r')
    self._settings.log.msg(msg_header + "self._fd_data: %d" % self._fd_data.fileno())

    # setup clock pin
    self._settings.log.msg(msg_header + "setting up clock-pin (%s)" % self._clock)
    ...
    self._settings.log.msg(msg_header + "self._fd_clock: %d" % self._fd_clock.fileno())
  ...
  def run(self):
    ...
    poll_obj = self._configure(msg_header)
    ...
    while True:
      ...
      for (fd,event) in poll_result:
        ...
        os.lseek(self._fd_data.fileno(),0,os.SEEK_SET)
        data = int(os.read(self._fd_data.fileno(),1))
        self._settings.log.msg(msg_header + "data is: %d" % data)
